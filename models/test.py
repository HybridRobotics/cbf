...
def plot_world(simulation, indexes, figure_name="world", local_traj_index=[]):
    ...
    ax.plot(global_path[:, 0], global_path[:, 1], "o--", color='grey', linewidth=1, markersize=2, transform=transform)
    ...
    for index in local_traj_index:
        ...
        ax.plot(local_path[:, 0], local_path[:, 1], "-", color='blue', linewidth=1.5, markersize=4, transform=transform)
        optimized_traj = optimized_trajs[index]
        ax.plot(optimized_traj[:, 0], optimized_traj[:, 1], "-", color='gold', linewidth=1.5, markersize=4, transform=transform)
...
def kinematic_car_rectangle_simulation_test():
    ...
    if control.dcbf_optimizer.ROBOT == "PENT":
        geometry_regions.add_geometry(KinematicCarTriangleGeometry(np.array([[0.15, 0.00], [0.03, 0.05], [-0.01, 0.02], [-0.01, -0.02], [0.03, -0.05]])))
        if control.dcbf_optimizer.MAZE == 1:
    ...
    if control.dcbf_optimizer.ROBOT == "TRI":
        geometry_regions.add_geometry(KinematicCarTriangleGeometry(0.75 * np.array([[0.14, 0.00], [-0.03, 0.05], [-0.03, -0.05]])))
        if control.dcbf_optimizer.MAZE == 1:
    ...
    if control.dcbf_optimizer.ROBOT == "L_SHAPE":
        geometry_regions.add_geometry(KinematicCarTriangleGeometry(0.4 * np.array([[0, 0.1], [0.02, 0.08], [-0.2, -0.1], [-0.22, -0.08]])))
        geometry_regions.add_geometry(KinematicCarTriangleGeometry(0.4 * np.array([[0, 0.1], [-0.02, 0.08], [0.2, -0.1], [0.22, -0.08]])))
        if control.dcbf_optimizer.MAZE == 1:
...
def create_env(env_type):
    if env_type == "s_path":
        ...
        obstacles.append(PolytopeRegion.convex_hull(s * np.array([[0.0, 2.0], [1.25, 3.875], [2.875, 3.125], [2.5, 2.25]])))
        obstacles.append(PolytopeRegion.convex_hull(s * np.array([[1, 4.75], [0.0, 5.0], [0.875, 7], [1.875, 6.375]])))
        obstacles.append(PolytopeRegion.convex_hull(s * np.array([[2.75, 1], [4.2, 3.25], [5.125, 3.75], [6.625, 2.5], [6.5, 1.0]])))
        obstacles.append(PolytopeRegion.convex_hull(s * np.array([[6.0, 7.0], [6, 6], [6.5, 7.0]])))
        obstacles.append(PolytopeRegion.convex_hull(s * np.array([[2.375, 4.875], [2.875, 5.875], [4.5, 5.875], [4.75, 4], [3.375, 4]])))
        ...
        obstacles.append(PolytopeRegion.convex_hull(s * np.array([[8.875, 3.125], [8.0, 5.5], [6.875, 6.375], [5.875, 5.875], [6.25, 4.375], [7.125, 3.5]])))
        return start, goal, grid, obstacles


if __name__ == "__main__":
    mazes = [1, 2]
    robot = ["RECT", "PENT", "TRI", "L_SHAPE"]
    for m in mazes:
        for r in robot:
